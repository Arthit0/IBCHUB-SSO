...
<div class="row">
	...
	<div class="col-md-8 col-md-none pl-0">
		...
		<div class="container-fluid d-flex flex-column justify-content-center align-items-center h-100">
			<?php if (!empty($state) && $state == "portal"): ?>
				<?php if ($res->res_code == "00"): ?>
					<div class="d-flex flex-column justify-content-center w-100">
						<img class="icon-img" src="<?php echo BASE_PATH; ?>asset/img/new-check-green.png" alt="">
						<h4 class="text-center  mt-4">ยืนยันสำเร็จ!</h4>
						<p class="text-center">ท่านได้ยืนยันอีเมลล์สำเร็จแล้ว</p>
						<a href="<?php echo BASE_PATH . _INDEX; ?>portal/profile" class="btn btn-login w-50">กลับไปยัง Portal</a>
					</div>
				<?php else: ?>
					<div class="d-flex flex-column justify-content-center w-100">
						...
						<div class="btn-group">
							<!-- <button role="button" onclick="send_mail(<?php echo $res->member_id ?>, '<?php echo $res->email ?>', '<?php echo $res->target ?>')" class="btn btn-send-again mitr-r">ส่งอีกครั้ง</button> -->
			...
			<?php else: ?>
				<?php if ($res['res_code'] == "00"): ?>
					<div class="d-flex flex-column justify-content-center w-100">
						...
						<p class="text-center">ท่านสามารถเข้าใช้งานระบบ DITP SSO</p>
						<!-- <a href="<?php echo BASE_PATH . _INDEX; ?>auth" class="btn btn-login w-50">เข้าสู่ระบบ</a>  ได้กรุณากดปุ่มเข้าสู่ระบบ-->
					</div>
				<?php else: ?>
					<div class="d-flex flex-column justify-content-center w-100">
						<img class="icon-img" src="<?php echo BASE_PATH; ?>asset/img/new-cross-red.png" alt="">
						...
						<p class="text-center"><?php echo $res['res_text'] ?></p> 
						<div class="btn-group">
							<!-- <button role="button" onclick="send_mail(<?php echo $res['res_result']['member_id'] ?>, '<?php echo $res['res_result']['email'] ?>', '<?php echo $res['res_result']['target'] ?>')" class="btn btn-send-again mitr-r">ส่งอีกครั้ง</button>
							<a href="<?php echo BASE_PATH . _INDEX; ?>auth" class="btn btn-login mitr-r">เข้าสู่ระบบ</a> -->
						</div>
					</div>
				<?php endif ?>
