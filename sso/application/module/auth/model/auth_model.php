...
class auth_model extends Model
{
  ...
  function moc_callback()
  {
    ...
    // $mocAccountUrl = 'grant_type=authorization_code&client_id=5111195809841&code='.$code.'&client_secret=eawh7mio9nkua42b0kx98t5s6nl7dv2j&redirect_uri=https%3A%2F%2Fsso.ditp.go.th%2Fsso%2Findex.php%2Fauth%2Fmoccallback';
      ...
      $response = curl_exec($curl);
        ...
        if($response){
          ...
          if($response_token){
            ...
            header( "location: https://sso.ditp.go.th/sso/test2.php" );
  ...
  function checkconsent($data){
    ...
    $client_name = $this->query($client_id);
     ...
     ($data[0]['type'] != 1)? $cid =  "" : $cid =  $data[0]['cid'];
     ($data[0]['type'] != 1)? $Name2 =  "" : $Name2 =  $userinfo->company_nameTh;
    ($data[0]['type'] == '1')? $ConsRoleId = 19 : $ConsRoleId = 14;
      $curl = curl_init();
        ...
        // pr($response); member_cid
        // pr(curl_getinfo($curl)); company_nameTh
        // pr(curl_error($curl));
      //   var_dump($pdpa);
      //   die();
  ...
  function userstaff($data){
    ...
    // $details->res_result->office_name = '';
    if ((empty($details->res_result->office_name) && $details->res_result->position_name != 'อธิบดี') && $details->res_result->position_name != 'รองอธิบดี') {
        return $return = ['status' => '01', 'data' => str_replace("api/",'',BASE_intranet).'profile/setting'];
    }
    //Get Ldap
    $curl = curl_init();
  ...
  function checkDBD($result,$data){
    ...
    $stmt->bind_param("s", $result['member_id']);
    $stmt->execute();
    $results = $stmt->get_result();
    $read = $results->fetch_assoc();
    ...
    foreach ($response->ns0getDataResponse->return->arrayRRow->childTables[0]->rows as $key => $value) {
      ...
      if(isset($value->columns)){
        ...
        if(count($explode2) == '2'){
          $DBD_names .= $explode2[0];
          $DBD_names .= $explode2[1];
        }else{
          $DBD_names .= $explode2[0];
        }
      }
      // if($read['status_contact_nationality'] == 1){
        $user_names .= $result['member_nameTh'];
        $user_names .= (empty($result['member_midnameTh']))?"":$result['member_midnameTh'];
  ...
  function model_district($data = ''){
    ...
    if(!empty($result)){
      $title = null;
      $title2 = null;
      $btn1 = lang('login_popup_later');
      $btn2 = lang('login_popup_attach_document');
      $btn3 = lang('login_popup_next_step');
      $btn4 = lang('login_popup_send_again');
      $btn5 = lang('login_popup_director_yes');
      $btn6 = lang('login_popup_attorney_no');
      $btn7 = lang('login_popup_acknowledge');
      $encounter = null;
      $text = null;
      $url = null;
      $urltype2 = null;
      ...
      switch ($result[0]['status_case']) {
        case '0':
          if($result[0]['director_status'] == '1'){
            $icon = "warning";
            $title = lang('login_popup_title1');
            $text = lang('login_popup_text_director1_case0');
            ...
            // หากท่านยังไม่ได้รับการยืนยันตัวตน ท่านมีสิทธิเข้าใช้งานระบบได้เพียง 3 วัน
            $foot = lang('login_popup_encounter');
          }
        ...
        case '1':
          $icon = "warning";
          $title = lang('login_popup_title1');
          $title2 = lang('login_popup_title2');
          $text = lang('login_popup_text_director2_case1');
        ...
        case '2':
          $icon = "warning";
          $title = lang('login_popup_title1');
          $title2 = lang('login_popup_title3');
          $text = lang('login_popup_text_director1_case2');
          $url = "auth/attach_file?q=".$result[0]['member_id']."&response_type=".$_SESSION['response_type']."&client_id=".$_SESSION['client_id']."&redirect_uri=".$_SESSION['redirect_uri'];
        ...
        case '3':
          $icon = "warning";
          $title = lang('login_popup_title4');
          $text = lang('login_popup_text_director1_case3');
          $foot = lang('login_popup_encounter');
          $status_case = $result[0]['status_case'];
          $next = true;
          break;
        ...
        case '5':
          $icon = "warning";
          $title = lang('login_popup_title5');
          $title2 = lang('login_popup_title2');
          $text = lang('login_popup_text_director1_case5');
          $url = "auth/attach_file?q=".$result[0]['member_id']."&response_type=".$_SESSION['response_type']."&client_id=".$_SESSION['client_id']."&redirect_uri=".$_SESSION['redirect_uri'];
          $urltype2 = "auth/director_form?q=".$result[0]['member_id']."&response_type=".$_SESSION['response_type']."&client_id=".$_SESSION['client_id']."&redirect_uri=".$_SESSION['redirect_uri'];
          $status_case = $result[0]['status_case'];
          $foot = lang('login_popup_encounter');
          break;
          default:
            if($result[0]['director_status'] == '1' && $result[0]['status_case'] != 99){
              $icon = "warning";
              $title = lang('login_popup_title1');
              $text = lang('login_popup_text_director1_case0');
              $url = "auth/attach_file?q=".$result[0]['member_id']."&response_type=".$_SESSION['response_type']."&client_id=".$_SESSION['client_id']."&redirect_uri=".$_SESSION['redirect_uri'];
              ...
              $foot = "";
              $status_case = 0;
            }else if($result[0]['director_status'] == '2' && $result[0]['status_case'] != 99){
              $icon = "warning";
              $title = lang('login_popup_title1');
              $text = 'ระบบจะส่งอีเมลเพื่อยืนยันฐานะผู้รับมอบอำนาจของท่านไปยังอีเมลของกรรมการและกรรมการของท่านจะต้องกรอกข้อมูลยืนยัน เพื่ออนุมัติว่าท่านเป็นผู้รับมอบอำนาจจริง ';
              // หากท่านยังไม่ได้รับการยืนยันตัวตน ท่านมีสิทธิเข้าใช้งานระบบได้เพียง 3 วัน
  ...
  function ck_login($user='',$pass='',$bool=false)
  {
    $return = [];
    if(!$bool) { 
      $user=$this->post(''.$_SESSION['username'].'');
      $pass=$this->post(''.$_SESSION['password'].'');
    }
    ...
    // die();
    $stmt = $this->db->prepare("SELECT * FROM tb_member WHERE cid = ? AND password = ? AND status != 2");
    $stmt->bind_param("ss",$user_name, $user_password_hash);
    $stmt->execute();
    $result = $stmt->get_result();
    $num = $result->num_rows;

    $ck_user_name = preg_match("#[a-zA-Z]+#", $user);
    if($ck_user_name == '1' && $num == 0){
      if(preg_match("/@ditp.go.th/i", $user) == '1'){
        $return['error'] = 'โปรดกรอก Usernam เดียวกันกับ Intranet (ไม่ต้องกรอก @ditp.go.th)';
        return $return;
      }
      $userstaff = $this->userstaff(['email' => $user.'@ditp.go.th','password' => $pass]);
    ...
    if($num > 0){ //-- เจอใน tb_member sso ปกติ --//
      ...
      if($val['type'] == '1'){
        ...
        if(!empty($district) && $district['status_case'] != "99" && !empty($district['status_case'])){
          ...
          $return = $this->checkconsent([$val,$return,$_SESSION['redirect_uri']]);
          if(empty($return['RedirectURL'])){
            $return = $this->get_data_return($val);
          }else{
            $return['url'] = $return['RedirectURL'];
          }
          ...
          setcookie('ssoid', $val['sso_id'], time() + (86400 * 30), "/");
          setcookie('type', $val['type'], time() + (86400 * 30), "/");
      ...
      if(count($result_ck_token_care) <= 0){ //-- auto regis care
        ...
        $code_care = $this->get_code($val['member_id'], $_SESSION['client_id']);
        $code = [
          'code_care' => $code_care
        ];  
      //   pr($code);
      ...
      if(count($code) > 0 ){
        $return_data = array_merge($return, ['status' => "00",'district' => $district,'pdpa' => $checkconsent, 'code' => $code]);
        // pr($val);
        // die();
        setcookie('ssoid', $val['sso_id'], time() + (86400 * 30), "/");
        setcookie('type', $val['type'], time() + (86400 * 30), "/");
      ...
      $sql_ck_cid = "SELECT 1 FROM tb_member WHERE cid = '$user_name' AND  status != 2";
      $result_ck_cid = $this->query($sql_ck_cid);
  ...
  function model_send_mail(){
    ...
    if(empty($callback)){
      empty($_SESSION['redirect_uri'])? $callback = BASE_URL.'portal/ck_portal': $callback = $_SESSION['redirect_uri'];
    }else{
      empty($callback)? $callback = BASE_URL.'portal/ck_portal': $callback = $callback;
    }
    if($email != "" && $member_id !="" && $target != ""){
      ...
      $data_token_reset = [
        ...
        'member_id' => $member_id,
        'target' => $target,
        'redirect_uri' => $callback,//empty($_SESSION['redirect_uri'])?BASE_URL.'portal/ck_portal':$_SESSION['redirect_uri'],
      ...
      $url = $this->path_web."auth/reset?q=".$token_reset."&ref_code=".$ref_code."&callback=".$callback."&response_type='token'";
      $mail->Body .= '<!DOCTYPE html>';
  ...
  function model_reset_save(){
    ...
    if($client_id == 'SS0047423'){
      $iPod    = stripos($_SERVER['HTTP_USER_AGENT'],"iPod");
      $iPhone  = stripos($_SERVER['HTTP_USER_AGENT'],"iPhone");
      $iPad    = stripos($_SERVER['HTTP_USER_AGENT'],"iPad");
      $Android = stripos($_SERVER['HTTP_USER_AGENT'],"Android");
      $Huawei   = stripos($_SERVER['HTTP_USER_AGENT'],"Huawei");
      if( $iPod || $iPhone ){
        $_SESSION['callback'] = $url;
      }else if($iPad){
        $_SESSION['callback'] = $url;
      }else if($Android || $Huawei){
        $_SESSION['callback'] = $url;
      }else{
        $_SESSION['callback'] = BASE_URL."portal/ck_portal";
      }
    }

    (empty($_SESSION['callback']))? $redirect_uri = $_SESSION['redirect_uri'] :  $redirect_uri =  $_SESSION['callback'];
    array_push($return , ['redirect_uri' => $url, 'client_id' => $client_id, 'response_type' => $type,'callback' => $redirect_uri ]);
    // var_dump($return,$_SESSION['callback'],$redirect_uri);redirect_uri
    // die;
    // $checkconsent = $this->checkconsent([$val,$return,$redirect_uri]);
  ...
  // exit;
  if ($info["count"]>0){
      $first_name = (isset($info[0]["cn"][0]))? $info[0]["cn"][0]:"";
      $last_name = (isset($info[0]["sn"][0]))? $info[0]["sn"][0]:"";
      $uid = (isset($info[0]["uid"][0]))? $info[0]["uid"][0]:"";
      $displayname = (isset($info[0]["displayname"][0]))? $info[0]["displayname"][0]:"";
  ...
  function model_director_form_send() {
    ...
    $tel_country = ($this->post('contact_director_tel_country') == '')? 'TH' : strtoupper($this->post('contact_director_tel_country'));
  ...
  function ck_laser_id ($data) {
    ...
    //     $return = [
    //       'code' => '01',
    //       'result' => 'error check laser 200' .$httpCode
    //     ];
    //     return json_decode(json_encode($return));
    // }
    curl_close($curl);
  ...
  function gen_input_name() {
    $gen_input_username = $this->random_char(5) . '' . $this->random_char(5);
    $gen_input_password = $this->random_char(5) . '' . $this->random_char(5);

    $_SESSION['username'] = $gen_input_username;
    $_SESSION['password'] = $gen_input_password;

    return [$gen_input_username,$gen_input_password];
  }
