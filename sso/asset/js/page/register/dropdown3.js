...
const change_district =(obj)=>{
    $(`#form_reg_noncompany [name^="bus[subdistrict]"]`).find('option').remove();
    $(`#form_reg_noncompany [name^="bus[subdistrict]"]`).html(`<option value="">กรุณาเลือกอำเภอ</option>`);
    $('.selectpicker').selectpicker('refresh');
    $(`#form_reg_noncompany [name^="bus[postcode]"]`).val('');
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $.each(result, function (key, value) {
                //console.log(value)
                $(`#form_reg_noncompany [name^="bus[subdistrict]"]`).append(`<option value="${value.id}">${value.name_th}</option>`);
                
            });
            $('.selectpicker').selectpicker('refresh');

            let select_subdistrict = $("#form_reg_noncompany [name='subdistrict']").val();
            let option_subdistrict = $("#form_reg_noncompany [name='bus[subdistrict]'] option");
...
const change_province = (obj) => {
    $(`#form_reg_noncompany [name="bus[district]"]`).find('option').remove();
    $(`#form_reg_noncompany [name="bus[subdistrict]"]`).find('option').remove();
    $(`#form_reg_noncompany [name="bus[postcode]"]`).val('');
    ...
    $.ajax({
        ...
        success: function (result) {
            $.each(result, function (key, value) {
                $(`#form_reg_noncompany [name="bus[district]"]`).append(`<option value="${value.id}">${value.name_th}</option>`);
                
            });
            $('.selectpicker').selectpicker('refresh');
            
            let select_district = $("#form_reg_noncompany [name='district']").val();
...
$(`#form_reg_noncompany [name^="bus[province]"]`).on('change', function () {
    change_province($(this));
})


/********** On Change amphures อำเภอ ***********/
$(`#form_reg_noncompany [name^="bus[district]"]`).on('change', function () {
    change_district($(this));
})

/********** On Change districe ตำบล ***********/
$(`#form_reg_noncompany [name^="bus[subdistrict]"]`).on('change', function () {
    var id = $(this).val();
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $(`#form_reg_noncompany [name^="bus[postcode]"]`).val(result[0].zip_code);
...
$(".radio_new_adress6").click(function () {
    ...
    $("#form_reg_noncompany [name='bus[district]']").val('');
    $("#form_reg_noncompany [name='bus[province]']").val('');
    $("#form_reg_noncompany [name='bus[district]']").val('');
    $("#form_reg_noncompany [name='bus[subdistrict]']").val('');
    $('.selectpicker').selectpicker('refresh');
});

const prop_dropdown =()=> {
    let select = $("#form_reg_noncompany [name='province']").val();
    let option = $("#form_reg_noncompany [name='bus[province]'] option");
